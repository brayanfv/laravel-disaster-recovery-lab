...
O ambiente possui a base Laravel/PHP, MySQL, MongoDB, Redis, Docker, Nginx, UFW, Fail2Ban, Cron, Portainer e Git funcional. Contudo, ainda não tem todos os componentes e dados necessários para exercitar uma recuperação completa e representativa: não há dados persistentes de uploads; as filas não têm trabalho real; o Scheduler ainda não possui disparo automático pelo Cron; HTTPS não está configurado; e não há local de armazenamento de backups definido.
...
| Laravel Scheduler | Tarefa `Append the scheduler disaster recovery marker` em `routes/console.php`, validada manualmente e por teste focal; gera log privado persistente | Tarefas de teste quando a aplicação exigir automação | Gap de ausência de tarefa representativa resolvido; não há disparo automático por Cron e não houve restore em máquina limpa | Decidir futuramente se o Cron deve executar `schedule:run` e validar recuperação em máquina limpa | Média |
...
9. **Média:** decidir se HTTPS e o disparo do Scheduler via Cron integram o laboratório.

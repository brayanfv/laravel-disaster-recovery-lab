...
| Portainer | Backup manual de `portainer_data`, SHA-256 local/remoto, transferência externa, restore em volume/container isolados e login com usuário existente no backup validados | Administração reprodutível com versão conhecida | Gap do fluxo manual de backup/restore resolvido para o laboratório; uso da tag mutável `latest`, scripts, retenção, monitoramento, secrets e restore em máquina limpa permanecem pendentes | Fixar versão/digest e automatizar o fluxo validado antes de declará-lo reproduzível | Alta |
...
| Laravel storage | Backup manual de `storage/app/private`, SHA-256 local/remoto, transferência externa e restore em diretório isolado validados; `DR_TEST_STORAGE_001.txt` e linhas de `scheduler-dr-test.log` recuperados | Arquivos persistentes de aplicação e uploads de teste | Gap do fluxo manual de backup/restore privado resolvido para o laboratório; faltam script, nomenclatura com data/hora, retenção, monitoramento, secrets, restore em máquina limpa e fluxo público | Automatizar o fluxo validado, testá-lo em máquina limpa e criar dados públicos somente se forem necessários | Resolvida para fluxo manual privado |
...
| Armazenamento de backups | Destino externo validado em `teste@172.23.1.115:/srv/backups/teste-deploy`; chave SSH dedicada, SCP e checksum SHA-256 dos fluxos manuais validados; MySQL automatizado validado até staging `.incomplete`, checksum remoto e promoção | Destino definido, dimensionado e acessível ao laboratório, com automação validada | Destino, autenticação, integridade manual, desenho e fluxo MySQL automatizado resolvidos; manifest global, retenção, lock global, Cron, logs/monitoramento dos demais componentes, restore automatizado e uploads públicos permanecem pendentes | Implementar e validar o desenho incrementalmente para os demais componentes antes de automatizar o agendamento | Resolvida para fluxos manuais e MySQL automatizado |
...
1. **Crítica:** concluir manifest global, secrets, retenção, lock, logs/monitoramento e os scripts dos demais componentes antes de ampliar a automação além do MySQL automatizado validado.

...
| MySQL | Backup lógico manual, transferência externa e restore isolado já validados; script local `backup-mysql.sh` validado em sucesso e falha proposital, com dump, SHA-256, log e cleanup de artefatos parciais | Banco reproduzível com dados de laboratório representativos e automação completa validada | Script local, geração de dump/checksum, logging e comportamento de falha/cleanup resolvidos; faltam transferência externa automatizada, `.incomplete` remoto, manifest global, retenção, lock global, Cron, secrets definitivos e restore em máquina limpa | Implementar e validar somente as etapas pendentes do fluxo MySQL antes de declará-lo automatizado de ponta a ponta | Resolvida para fluxo local e manual |
...
| Armazenamento de backups | Destino externo validado em `teste@172.23.1.115:/srv/backups/teste-deploy`; chave SSH dedicada, SCP e checksum SHA-256 dos fluxos manuais validados; script MySQL local testado com sucesso e falha | Destino definido, dimensionado e acessível ao laboratório, com automação validada | Destino, autenticação, integridade manual, desenho e script MySQL local resolvidos; transferência externa automatizada, `.incomplete` remoto, manifest global, retenção, lock global, Cron, logs/monitoramento dos demais componentes, restore automatizado e uploads públicos permanecem pendentes | Implementar e validar o desenho incrementalmente antes de automatizar o agendamento | Resolvida para fluxos manuais e MySQL local |
...
1. **Crítica:** concluir transferência externa automatizada, `.incomplete` remoto, manifest global, secrets, retenção, lock, logs/monitoramento e os scripts dos demais componentes antes de ampliar a automação além do MySQL local validado.

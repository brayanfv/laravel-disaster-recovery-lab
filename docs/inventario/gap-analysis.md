...
| Dado persistente | MySQL `teste_deploy`, volumes `portainer_data`, `mongodb_data` e `redis_data`, futuros arquivos em `storage/app` | Há persistência validada no MongoDB e Redis, mas faltam decisões de backup/restore, dados fictícios de aplicação e uploads. |

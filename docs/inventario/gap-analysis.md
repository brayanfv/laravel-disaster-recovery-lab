...
| Armazenamento de backups | Destino externo validado em `teste@172.23.1.115:/srv/backups/teste-deploy`; chave SSH dedicada, SCP e checksum SHA-256 dos fluxos MySQL, MongoDB, Redis, Laravel storage e `portainer_data` validados; desenho de automação documentado | Destino definido, dimensionado e acessível ao laboratório, com automação validada | Destino, autenticação, integridade manual e desenho inicial resolvidos; implementação de scripts, secrets, retenção, logs, monitoramento, Cron de backup, restore automatizado e uploads públicos permanecem pendentes | Implementar e validar o desenho antes de ampliar os backups reais ou automatizar o agendamento | Resolvida para fluxos manuais e design |
...
1. **Crítica:** implementar e validar scripts, secrets, nomenclatura, retenção, logs, monitoramento e automação a partir do desenho documentado antes de ampliar os backups reais além dos fluxos manuais MySQL, MongoDB, Redis, Laravel storage e Portainer validados.

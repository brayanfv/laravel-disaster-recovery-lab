...
| Armazenamento de backups | Destino externo validado em `teste@172.23.1.115:/srv/backups/teste-deploy`; chave SSH dedicada, SCP e checksum SHA-256 do fluxo MySQL validados | Destino definido, dimensionado e acessível ao laboratório | Destino, autenticação e integridade manual do fluxo MySQL resolvidos; scripts, retenção, nomenclatura, monitoramento e backups dos demais componentes permanecem pendentes | Definir automação e política operacional antes de ampliar os backups reais | Resolvida para destino, autenticação e fluxo MySQL |
...
1. **Crítica:** definir scripts, nomenclatura, retenção, monitoramento e automação antes de ampliar os backups reais além do fluxo manual MySQL validado.

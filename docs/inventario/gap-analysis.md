...
| Armazenamento de backups | Destino externo validado em `teste@172.23.1.115:/srv/backups/teste-deploy`; chave SSH dedicada, SCP e checksum SHA-256 dos fluxos MySQL, MongoDB, Redis, Laravel storage e `portainer_data` validados | Destino definido, dimensionado e acessível ao laboratório | Destino, autenticação e integridade manual dos fluxos persistentes validados resolvidos; scripts, retenção, nomenclatura, monitoramento e uploads públicos permanecem pendentes | Definir automação e política operacional antes de ampliar os backups reais | Resolvida para os fluxos persistentes atuais |
...
1. **Crítica:** definir scripts, nomenclatura, retenção, monitoramento e automação antes de ampliar os backups reais além dos fluxos manuais MySQL, MongoDB, Redis, Laravel storage e Portainer validados.

...
Este é um documento de design. As implementações incrementais de MySQL, MongoDB, Redis, Laravel storage e Portainer já existem e foram validadas em execução real isolada. MySQL e MongoDB validaram staging local, checksum, transferência remota `.incomplete`, validação remota e promoção; Redis também validou `SAVE`, parada controlada, archive somente-leitura do volume e reinício obrigatório; Laravel storage validou archive relativo, restore isolado e recuperação do marcador persistente; e Portainer validou parada controlada, archive somente-leitura sem Docker socket, restart, permissões, transferência, promoção e restore isolado do conteúdo do volume. O orquestrador `backup.sh` também foi validado na execução geral `2026-09-24_142925`: um único `RUN_ID`, os cinco componentes em modo orquestrado, manifesto global, validação remota e promoção única. Lock global foi validado em teste real. A retenção de sete restore points foi validada de modo não destrutivo no `RUN_ID` `2026-09-24_165238` e, de modo destrutivo, em uma raiz remota isolada com nove fixtures válidos, dos quais somente os dois mais antigos foram removidos. As evidências estão em [backup-script-mysql.md](backup-script-mysql.md), [backup-script-mongodb.md](backup-script-mongodb.md), [backup-script-redis.md](backup-script-redis.md), [backup-script-laravel-storage.md](backup-script-laravel-storage.md), [backup-script-portainer.md](backup-script-portainer.md) e [backup-orchestrator.md](backup-orchestrator.md). O wrapper e a entrada diária de Cron existem; a execução automática pelo daemon foi validada no `RUN_ID` `2026-09-25_160501`. Restore automatizado não existe. Os procedimentos manuais documentados continuam sendo a referência.
...
- `backup.sh` é o orquestrador: cria o identificador único, prepara staging e logs, controla a ordem, consolida checksums, transfere o manifesto e define o resultado final.
...
No estado atual, existem `backup.sh`, `backup-mysql.sh`, `backup-mongodb.sh`, `backup-redis.sh`, `backup-laravel-storage.sh`, `backup-portainer.sh` e `lib/common.sh`. Os cinco scripts de componente foram validados em execução real isolada, e o orquestrador foi validado ponta a ponta na execução geral `2026-09-24_142925`.
...
- em uma execução completa, `backup.sh` gera ou recebe o `RUN_ID`, prepara uma única vez o staging remoto e o repassa a todos os scripts de componente com `DR_ORCHESTRATED=1`;
...
O orquestrador usa `mv` entre os diretórios `.incomplete/<RUN_ID>` e `<RUN_ID>` no mesmo destino remoto. Ele confirma antes que o diretório incompleto existe, que o final não existe e que todos os arquivos esperados estão presentes. A execução geral `2026-09-24_142925` validou esse fluxo completo no laboratório.
...
Cada artefato tem checksum SHA-256 individual. Após a criação e a validação remota dos cinco componentes, o orquestrador gera `manifest.sha256` na raiz local da execução contendo os hashes dos artefatos finais, com caminhos relativos, por exemplo:
...
O manifesto recebe modo `600`, é transferido para `.incomplete/<RUN_ID>`, recebe modo `600` também no destino e é validado remotamente antes da única promoção. A execução `2026-09-24_142925` confirmou os cinco hashes localmente e remotamente, com artefatos, checksums e manifesto em modo `600`. SHA-256 confirma integridade dos bytes transferidos; ele não substitui uma cópia externa, não fornece criptografia e não elimina a necessidade de um restore de teste.

Como hardening comum implementado nos scripts isolados de MySQL, MongoDB, Redis, Laravel storage e Portainer, cada artefato e seu checksum devem terminar em modo `600` tanto no staging local quanto no diretório remoto `.incomplete`, antes da validação de checksum e promoção. A aplicação é explícita, sem `chmod` recursivo: somente os arquivos esperados recebem a permissão. As execuções MongoDB `2026-09-22_163621`, Redis `2026-09-22_171547`, Laravel storage `2026-09-22_172710` e Portainer `2026-09-24_134725` validaram os quatro modos — artefato/checksum local e artefato/checksum remoto — em `600`, além da sequência SCP → `chmod 600` remoto → checksum → promoção. A execução geral `2026-09-24_142925` confirmou essa política também para MySQL e para os cinco componentes sob o mesmo `RUN_ID`, incluindo `manifest.sha256` local e remoto em modo `600`.
...
Os scripts implementados usam `set -Eeuo pipefail` e `umask 077`. O tratamento explícito de estado e de reinício obrigatório permanece nos componentes que exigem parada controlada. A execução geral integrada foi validada; Redis e Portainer voltaram ao estado `Up`, e `DR_TEST_REDIS_001` permaneceu válido.
...
Os logs ficam fora do repositório, em `/srv/teste-deploy-data/backup-logs/`. Cada execução geral já registra um arquivo por `RUN_ID`. O wrapper `run-backup-cron.sh` adiciona stdout e stderr em `cron-backup.log`, mantendo um ponto simples de consulta operacional. Além da validação manual `2026-09-24_173106`, a execução automática pelo daemon foi validada no `RUN_ID` `2026-09-25_160501`, com manifesto remoto, promoção e retenção concluídos. Cada execução deve registrar, no mínimo:
...
A ordem implementada pelo orquestrador é:

1. MySQL;
2. MongoDB;
3. Redis;
4. Laravel storage privado;
5. Portainer.

MySQL e MongoDB são copiados primeiro porque seus métodos manuais validados não exigem parada controlada no laboratório. Redis e Portainer preservam suas janelas controladas de parada e restart; Laravel storage é copiado por filesystem sem parar a aplicação. Não há paralelismo, e cada componente precisa concluir antes do próximo iniciar.
...
`backup.sh` implementa lock global com `flock -n` em `/srv/teste-deploy-data/backup-staging/.backup.lock`. A aquisição ocorre antes da criação de staging remoto e impede apenas uma segunda execução geral; scripts de componente isolados não usam esse lock. A contenção foi testada localmente e validada em teste real no laboratório.

A política provisória implementada é manter 7 restore points completos. `BACKUP_RETENTION_COUNT` pode ser configurado entre `1` e `365`. A retenção é executada somente depois que uma nova execução foi promovida; ela seleciona exclusivamente diretórios regulares não simbólicos, no nível direto de `BACKUP_REMOTE_ROOT`, com `RUN_ID` válido e `manifest.sha256` regular. Os nomes são ordenados de forma decrescente, os mais recentes são preservados e apenas excedentes validados são removidos por caminho explícito.

`.incomplete/`, arquivos soltos, links simbólicos, nomes inválidos e diretórios sem manifesto não são candidatos. Uma falha de retenção posterior à promoção gera aviso, mas não invalida o novo restore point. A seleção foi confirmada sem remoção no `RUN_ID` `2026-09-24_165238` (`2` restore points, `keep_count=7`). A remoção foi validada em `/srv/backups/teste-deploy-retention-test`, raiz remota isolada: dentre nove diretórios válidos, foram removidos apenas `2026-01-01_010101` e `2026-01-02_010101`; os sete mais recentes e todas as entradas não elegíveis permaneceram preservados. O destino real `/srv/backups/teste-deploy` não foi usado como alvo destrutivo nesse teste.
...
Os procedimentos manuais validados permanecem a referência para MySQL, MongoDB, Redis, Laravel storage e `portainer_data`. O [runbook detalhado](full-restore-runbook.md) e o teste em máquina limpa já foram concluídos com o restore point `2026-09-24_173106`; o restore automatizado e a portabilidade do pipeline continuam pendentes.
...
- Validar os demais cenários de falha ainda não injetados individualmente.
- Avaliar política para execuções perdidas: Cron tradicional não recupera automaticamente a tarefa se host/serviço estiver desligado no horário; considerar futuramente `systemd timer` persistente ou equivalente.
...
- Formalizar o reprovisionamento do pipeline de backup em novo host e projetar restore automatizado a partir do runbook manual validado.

## Situação atual

O desenho da automação está implementado e validado no laboratório. Os backups isolados de MySQL, MongoDB, Redis, Laravel storage e Portainer foram validados ponta a ponta com staging `.incomplete`, checksum remoto e promoção; o MongoDB também validou o uso da configuração temporária `--config` sem senha em argv e a remoção dos temporários do container. A execução geral `2026-09-24_142925` validou orquestrador, `RUN_ID` único, staging remoto único, manifesto global, verificação remota dos cinco artefatos e promoção única. Lock e retenção foram validados. O wrapper de Cron e `cron-backup.log` foram validados manualmente no `RUN_ID` `2026-09-24_173106` e automaticamente pelo daemon no `RUN_ID` `2026-09-25_160501`, com os cinco componentes, manifesto remoto, promoção e retenção em sucesso. O restore point `2026-09-24_173106` foi restaurado e validado manualmente em máquina limpa, incluindo dados, aplicação, Nginx, PHP-FPM, Vite, autenticação e Scheduler por Cron. Não há alertas externos, monitoramento ativo, gestão formal de secrets ou restore automatizado; Cron tradicional também não recupera execuções perdidas quando o host está desligado. O DR geral permanece parcialmente validado.

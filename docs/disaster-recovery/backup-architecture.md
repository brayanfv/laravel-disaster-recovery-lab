...
Esta é a arquitetura inicial de backup e disaster recovery do laboratório. Os fluxos manuais de backup e restore de MySQL, MongoDB, Redis e Laravel storage foram validados com staging local, SHA-256, transferência externa e restores isolados. Ainda não há scripts, automação, backup dos demais componentes, criptografia, retenção ou restore em máquina limpa.
